...
dt=double(1e-3); Nt=20; %Define time step and number
...
phi1=0; phi2=0;
psi_1=(1i*c1/sqrt(2)+...
sqrt(1-c1^2/2).*tanh((x-x0-c1*t).*sqrt(2-c1^2)./2))*exp(1i*phi1);
psi_2=(1i*c2/sqrt(2)+...
    sqrt(1-c2^2/2).*tanh((x+x0-c2*t).*sqrt(2-c2^2)./2))*exp(1i*phi2);
psi_0=abs(psi_1).^2+abs(psi_2).^2+...
    2.*abs(psi_1).*abs(psi_2).*cos(phi2-phi1);

for itime=1:Nt %Time-stepping with split-step Fourier method
psi1=(1i*c1/sqrt(2)+...
    sqrt(1-c1^2/2).*tanh((x-x0-c1*t).*sqrt(2-c1^2)./2))*exp(1i*phi1);
psi1=psi1.*exp(-0.5*1i*dt*(V-abs(psi1).^2));
...
psi2=(1i*c2/sqrt(2)+...
    sqrt(1-c2^2/2).*tanh((x+x0-c2*t).*sqrt(2-c2^2)./2))*exp(1i*phi2);
psi2=psi2.*exp(-0.5*1i*dt*(V-abs(psi2).^2));
...
psi_diff=zeros(1,size(psi2,2));

% Gráficas
psi=abs(psi1).^2+abs(psi2).^2+...
    2.*abs(psi1).*abs(psi2).*cos(phi2-phi1);
psi_col=abs(psi1_col).^2+...
    abs(psi2_col).^2+2.*abs(psi1_col).*abs(psi2_col).*cos(phi2-phi1);
spacetime=abs(spacetime1).^2+...
    abs(spacetime2).^2+2.*abs(spacetime1).*abs(spacetime2).*cos(phi2-phi1);
figure(1)
subplot(3,1,3)
plot(x,psi_0,'b',x,psi_col,'r',x,psi,'g--','LineWidth',1.5);
legend('\psi(x,0)','\psi(x,t_c)','\psi(x,t_f)');...
